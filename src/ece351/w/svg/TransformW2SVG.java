	...
	public static void transform(final WProgram wp, final PrintWriter out) {
		...
		for (final Waveform w : wp.waveforms) {
			...
			for (final String bit : w.bits) {
				if(y_prev == y_mid) {
				// the wave form just start 
					if (bit=="1") {
						out.println(Line.toSVG(x,y_mid,x,y_mid-50));
						out.println(Line.toSVG(x,y_mid-50,x+100,y_mid-50));
						y_prev = y_mid - 50;
						x = x+100;
				...
				// last bit is 1
					...
					if(bit=="0") {
						out.println(Line.toSVG(x,y_mid-50,x,y_mid+50));
						out.println(Line.toSVG(x,y_mid+50,x+100,y_mid+50));
						y_prev = y_mid + 50;
						x = x+100;
					}
