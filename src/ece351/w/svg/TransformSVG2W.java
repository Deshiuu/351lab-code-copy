...
public final class TransformSVG2W {
	...
	public static final WProgram transform(final PinsLines pinslines) {
		...
		for (Pin mypins : pins) {
			List<Line> oneWavLines = new ArrayList<Line>();
			for (Line mylines : lines) {
				if(mylines.y1 == mylines.y2) {
					if (mylines.y1 == mypins.y || mylines.y1==mypins.y+50 || mylines.y1==mypins.y-50) {
						oneWavLines.add(mylines);
					}
				}
			}
			Collections.sort(oneWavLines, COMPARE_X);
			a = a.append(transformLinesToWaveform(oneWavLines, mypins));
		}
	 ...
	 * @see Pin#id
	 */
	private static Waveform transformLinesToWaveform(final List<Line> lines, final Pin pins) {
    	...
    	for(Line l:lines) {
    		if (l.y1 == pins.y+50) {
    			bits1 = bits1.append("0");
    		}
    		else if (l.y1 == pins.y-50) {
    			bits1 = bits1.append("1");
    		}
    	}
