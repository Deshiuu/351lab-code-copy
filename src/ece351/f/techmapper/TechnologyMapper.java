...
//--------------------------------------------------------------------------				
				...
				FProgram fp1 = new FProgram();
				FProgram fp2 = new FProgram();
				for (AssignmentStatement f : program.formulas) {
					int x = 0;
					for (AssignmentStatement g : fp.formulas) {
						if (f.expr.equals(g.expr)){
							x++;
							if(x==1) {
								fp2 = fp2.append(f);
	...
	// remove identical formulas;	
	//--------------------------------------------------------------------
				//FProgram fp1 = new FProgram();
				for (AssignmentStatement h : fp1.formulas) {
					visitVar(h.outputVar);
					Expr e;
					e = traverseExpr(h.expr);
					edge(e, h.outputVar);
					for(AssignmentStatement j : fp2.formulas) {
						if (h.expr.equals(j.expr)) {
							visitVar(j.outputVar);
							edge(e, j.outputVar);
						}
					}
				}
...
// TODO: short code snippet
		final Expr e2  = substitutions.get(e);
		final AndExpr e3 = (AndExpr) e2;
		node(e3.serialNumber(), e3.toString(), "../../gates/and_noleads.png");
		edge(e3.left, e3);
		edge(e3.right, e3);
		return e;
	}
...
// TODO: short code snippet
		final Expr e2  = substitutions.get(e);
		final OrExpr e3 = (OrExpr)e2;
		node(e3.serialNumber(), e3.toString(), "../../gates/or_noleads.png");
		edge(e3.left, e3); 
		edge(e3.right, e3);
		return e;
	}
