...
public final class FRecursiveDescentParser implements Constants {
    ...
    Expr term() {
    	...
    	while(!lexer.inspect(";") && !lexer.inspect(")") ) {
	    	if (lexer.inspect("and")) {
	    		lexer.consume("and");
	    		cur = new AndExpr(cur, factor());
	    	}
	    	else if (lexer.inspect("or")) {
	    		lexer.consume("or");
	    		cur = new OrExpr(cur, factor());
	    	}
	    	else {
	    		cur = factor();
	    	}
    ...
    Expr factor() { 
		...
		if (peekConstant()) {
			...
			lexer.consume("false");
			exp = ConstantExpr.make(false);
		}
		else if (lexer.inspect("true")) {
			lexer.consume("true");
			exp = ConstantExpr.make(true);
		}
