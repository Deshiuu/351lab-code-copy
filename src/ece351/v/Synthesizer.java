...
public final class Synthesizer extends PostOrderExprVisitor {
	...
	private FProgram synthesizeit(final VProgram root) {	
		...
		for(DesignUnit a : root.designUnits){
			varPrefix = a.arch.entityName;
			for(Statement s : a.arch.statements){
				if(s instanceof AssignmentStatement) {
					result = result.append(newassign(s));
				}
				else{
					for(Statement s1 : ((Process)s).sequentialStatements){
						if(s1 instanceof AssignmentStatement){
							result = result.append(newassign(s1));
						}
						else{
							result = result.appendAll((FProgram)this.implication((IfElseStatement)s1));
						}
					}
				}
			}
...
// TODO: longer code snippet
	...
	private AssignmentStatement newassign(final Statement s) {
		VarExpr e = (VarExpr)traverseExpr((Expr)((AssignmentStatement)s).outputVar);
		Expr e2 = traverseExpr(((AssignmentStatement)s).expr);
		AssignmentStatement a = new AssignmentStatement(e,e2);
		return a;
	}
	...
	private FProgram implication(final IfElseStatement statement) {
		...
		condCount++;
		FProgram result = new FProgram();
		AssignmentStatement a1 = new AssignmentStatement(new VarExpr(conditionPrefix+condCount),
														traverseExpr(statement.condition));
		AssignmentStatement a2 = new AssignmentStatement(varPrefix + ifb.outputVar, 
														new OrExpr(new AndExpr(new VarExpr(conditionPrefix+condCount), 
														traverseExpr(ifb.expr)), 
														new AndExpr(new NotExpr(new VarExpr(conditionPrefix+condCount)),
														traverseExpr(elb.expr))));
		result = result.append(a1);
		result = result.append(a2);
		return result;
		// build result
		// TODO: longer code snippet
		//---------------------------------------------------------------------------------------------
